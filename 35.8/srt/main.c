...
void* multi(void *arg1) {
  ...
  /* for every pixel */
  for(int px = *mrow; px<width; px= px + nthreads )
    {
...
main( int argc, char **argv )
{
    ...
    */
    ...
    //I have to create pointers to every row
    int *prow = (int*) malloc(sizeof(int*) * nthreads);
    for (int i = 0; i < nthreads; i++) {
      prow[i] = i;
    }
    
    //I create my multithreads here
    for (int i = 0; i < nthreads; i++) {
      int ans = pthread_create(&tids[i], NULL, multi, &prow[i]);
      if (ans != 0) {
	fprintf(stderr, "Couldn't allocate memory.\n");
	exit(-1);
      }
    ...
    //make sure they all join together
    for (int i = 0; i < nthreads; i++) {
